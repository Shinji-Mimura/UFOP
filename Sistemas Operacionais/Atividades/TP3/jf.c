...
/*
   Semaforo Binário
   - Concede entrada a região crítica para as threads
   - Possui apenas dois estados: 0 (bloqueado) e 1 (desbloqueado) 
*/
sem_t mutex;
...
void agarraGarfo(int nfilosofo)
{
   ...
   testar(nfilosofo);
   sem_post(&mutex);        // Incrementar (desbloqueia) o semaforo. Se o valor do semaforo se tornar maior que 0, então outro processo ou thread sera bloqueado pelo sem_wait
   sem_wait(&S[nfilosofo]); // Decrementa (bloqueia) o semaforo
   sleep(1);
}
